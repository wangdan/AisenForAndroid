...
public class BitmapProcess {
	...
	private byte[] getBitmapFromDiskCache(String url, String key, FileDisk fileDisk, ImageConfig config) throws Exception {
		...
		if (config.getProgress() != null)
			config.getProgress().receiveLength(inputStream.available());
		...
		while ((readLen = inputStream.read(buffer)) != -1) {
			readBytes += readLen;
			if (config.getProgress() != null)
				config.getProgress().receiveProgress(readBytes);
	...
	public MyBitmap compressBitmap(byte[] bitmapBytes, String url, int flag, ImageConfig config) throws Exception {
		boolean writeToComp = config.getCornerSize() > 0;
		...
		if ((flag & 0x01) != 0) {
			myBitmap = new MyBitmap(BitmapDecoder.decodeSampledBitmapFromByte(bitmapBytes), bitmapType, url);
			return myBitmap;
		}
		...
		// 对图片做圆角处理
		if (bitmapType != BitmapType.gif && config.getCornerSize() > 0) {
			bitmap = BitmapUtil.setImageCorner(bitmap, config.getCornerSize());
		...
		return new MyBitmap(bitmap, bitmapType, url);
	}
