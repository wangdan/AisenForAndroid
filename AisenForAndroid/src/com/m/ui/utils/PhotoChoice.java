...
public class PhotoChoice {
	...
	private PhotoChoice() {
		picMaxDecodeWidth = SystemUtility.getScreenWidth() * 5;
		picMaxDecodeHeight = SystemUtility.getScreenHeight() * 3;
	}
		...
		if (resultCode == Activity.RESULT_OK) {
			...
			if (requestCode == PHONE_IMAGE_REQUEST_CODE) {
				...
				case byteType:
					try {
						InputStream is = mContext.getContentResolver().openInputStream(data.getData());
						choiceListener.choiceByte(FileUtility.readStreamToBytes(is));
					} catch (Exception e) {
						e.printStackTrace();
					}
					break;
			...
			// 相机拍摄
				...
				case bitmapType:
					Bitmap bitmap = null;
					try {
						byte[] datas = FileUtility.readStreamToBytes(new FileInputStream(tempFilePath + tempFileName));
						bitmap = BitmapDecoder.decodeSampledBitmapFromByte(datas);
					} catch (Exception e) {
						e.printStackTrace();
					} finally {
				...
				case byteType:
					try {
						choiceListener.choiceByte(FileUtility.readFileToBytes(new File(tempFilePath + tempFileName)));
					} catch (Exception e) {
						e.printStackTrace();
					} finally {
						// 删除原文件
